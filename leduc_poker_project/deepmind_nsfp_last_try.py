...
def train(args):
    ...
    # Per JAX, NFSP usa internamente PRNGKey fisso o basato su np.random
    
    setup_gpu()
    start_dashboard()
    local_logger = LocalLogger(args.logger_prefix)
    ...
    env.seed(args.seed) # Questo fissa il seed per il mazziere (chance nodes)
    num_actions = env.action_spec()["num_actions"]
    info_state_size = env.observation_spec()["info_state"][0]

    print("\n")
    print(f"Numero di azioni: {num_actions}")
    print(f"Dimensione dello stato: {info_state_size}")
    print("\n")

    # 2. Setup Agenti
    if args.algo == "nfsp":
        agents = [
            nfsp.NFSP(idx, info_state_size, num_actions, [args.neurons],
        ...
        agents = [
            dqn_jax.DQN(idx, info_state_size, num_actions, [args.neurons])
            for idx in range(2)
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--neurons", type=int, default=64, help="Numero di neuroni nello strato nascosto")
    parser.add_argument("--logger_prefix", type=str, default="nsfp_swapped", help="Prefisso per il nome della run nel logger")
